...
public abstract class BranchI extends Instr
{
    public BranchI(int row, TAC instr, String op, Map<String, Integer> look, boolean negate)
    {
        ...
        this.blocks.add(new Chain(row, this.blocks.size(), execScore + "a" + execData + instr.op1.id + execValue));
        this.blocks.add(new Chain(row, this.blocks.size(), execScore + "b" + execData + instr.op2.id + execValue));

        // NEXT INSTR

        String cond = "score a calc " + op + " b calc run ";
        ...
        String branch = JUMP.component(row, this.blocks.size(), instr.op3, look).get(1);
